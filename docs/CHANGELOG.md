...
### Changed
- **The Java baseline is now 17, down from 25** ([#3](https://github.com/vidyalai-in/claude-agent-sdk-java/issues/3)). The SDK is compiled with `<release>17</release>`, so it runs on any JDK from 17 onwards. Nothing in the published API changed: sealed interfaces and records are Java 17 features, so consumers on JDK 21+ can still write exhaustive pattern switches over `Message`, `ContentBlock` and `ThinkingConfig` exactly as before. The Java 25 requirement rested on two things — pattern-matching `switch` in six internal methods, and virtual threads — and neither needed to be a requirement of the *consumer's* toolchain.
- **Virtual threads are now selected at runtime rather than required at compile time.** All SDK threads and executors come from a new internal `Threads` helper, which resolves `Thread.ofVirtual()` and `Executors.newThreadPerTaskExecutor` reflectively once into `static final` method handles. On Java 21+ the behaviour is identical to before. On 17-20 it falls back to named daemon platform threads from an unbounded cached pool. Thread names (`QueryHandler-Reader-0`, `SPCTransport-Stderr-0`, `session-store-0`, …) are unchanged on both paths, so thread dumps read the same way. Set `-Dclaude.sdk.virtualThreads=false` to force the platform path on any JDK.
  - The fallback is deliberately **unbounded**, not a fixed pool. `QueryHandler`'s control executor parks a thread for the duration of an SDK MCP tool call, and the `notifications/cancelled` that ends it arrives as a *separate* control request; under a bounded pool that cancellation would queue behind the call it exists to cancel and deadlock.
  - The fallback threads are **daemon** threads, because virtual threads always are. Anything else would turn every SDK thread into a JVM-exit blocker for applications on a 17 runtime.
- **`SessionStoreExecutor`'s documented default is now runtime-dependent.** It is still one thread per task named `session-store-<n>`; those are virtual threads on Java 21+ and daemon platform threads on 17-20. Anyone who called `setDefault(...)` is unaffected.
- **The SDK jar now declares `Automatic-Module-Name: in.vidyalai.claude.sdk`**, so JPMS consumers get a stable module name instead of one derived from the jar filename.

### Fixed
- **Intermittent `cannot find symbol` build failures.** The root cause was `<useIncrementalCompilation>false</useIncrementalCompilation>` in the compiler plugin config. The flag is inverted: setting it to `false` does not disable incremental compilation, it switches the plugin from "recompile the module when anything changed" to timestamp-based stale-source selection, which hands javac a partial source set and relies on javac's *implicit* compilation to supply the rest. That is unsound for Java — a file whose own timestamp did not move is never recompiled even when a type it depends on changed, and any class outside the reference closure of the "stale" files is never written at all. Observed directly: `mvn clean test-compile` reported "Compiling 1 source file" for a module with 47 test sources, and touching a main source produced "Nothing to compile". `SdkVersion.java` kept it permanently armed, because `templating-maven-plugin` regenerates it on every run, so it was always the single stale file javac was given. Removing the flag makes every build compile the full source set.

- **Listing sessions from a `SessionStore` deadlocked under a bounded executor.** `SessionStores.deriveInfosViaLoadConcurrent` fans out one `loadAsync` per session, and the default `loadAsync` submits to `SessionStoreExecutor`. Because the fan-out itself ran as a task on that same executor and then blocked on `.get()`, it was waiting for a slot it was occupying. With the default unbounded thread-per-task executor that merely costs a thread, but with a bounded one — the configuration `SessionStoreExecutor`'s own javadoc offers as an example (`setDefault(Executors.newFixedThreadPool(8))`) — every worker parked and `listSessionsFromStore` never returned. The loads are now issued from the calling thread, with the semaphore parking the caller rather than an executor thread; concurrency is bounded exactly as before at 16. `SessionStoreBoundedExecutorTest` covers single-thread, fixed-pool, paginated and default-executor configurations, and each test carries a `SEPARATE_THREAD` `@Timeout` so a regression fails the build instead of hanging it.

- **Transcript mirroring was dropped entirely under a bounded executor.** Same self-submission shape as the listing bug above: `TranscriptMirrorBatcher` ran each drain as a task on the configured `SessionStoreExecutor`, and the drain then blocked on the `appendAsync` it submitted — which the default implementation submits to that same executor. With a bounded executor the append could never be scheduled, so every flush waited out `sendTimeoutMs` (60s by default) and the batch was dropped through `onError`; `close()` blew its own 5-second budget too. Drains now run on a small shared, daemon, thread-per-task executor owned by the SDK. The adapter itself still runs on the configured executor — that is the caller's lever for bounding their own store work, and only the SDK's blocking coordinator moved off it. `TranscriptMirrorBatcherBoundedExecutorTest` covers flush, repeated flushes and `close()` under a single-threaded executor, and asserts the adapter still runs on the caller's pool.

- **A racy assertion in `TranscriptMirrorBatcherTest`.** `receiveLoop_eagerFlushMode_appendsPerFrameBeforeResult` failed 14 times in 25 runs on Java 17. Two bugs, both in the test rather than in the batcher: its recording `SessionStore` collected entries into a plain `ArrayList` mutated from two executor threads, which corrupts `size()` rather than merely reordering; and it asserted `appendCalls == 2` on the assumption that `flush()` joins an eager drain already in flight, which it does not. It passed on 21+ only because virtual threads happened to win that race. The list is now synchronized and the second append is awaited explicitly, so the test still fails if EAGER ever coalesces frames — which is what it exists to catch.

### Added
- **A CI workflow** (`.github/workflows/ci.yml`) building and testing on JDK 17, 21 and 25 on every push and pull request. The repository previously had no build workflow at all — only a `workflow_dispatch` publish job — so nothing caught a baseline regression. The 17 leg is the baseline; 21 and 25 cover the runtimes where `Threads` takes the virtual-thread path, which `--release 17` alone cannot check. A final step asserts the emitted bytecode is class-file major version 61.
- **`SealedExhaustivenessTest`**, the tripwire that replaces the compiler's exhaustiveness check. A pattern switch over a sealed type with no `default` fails to compile the moment a new subtype is permitted; an `instanceof` chain silently falls through to its `else` instead. This test fails on any new permitted subtype of `Message`, `ContentBlock`, `SDKControlRequestData`, `ControlResponseData`, `PermissionResult` or `ThinkingConfig`, and names every dispatch site to update.
- **`ThreadsTest`**, which exercises both threading paths on any JDK. It reloads `Threads` in a throwaway classloader with `claude.sdk.virtualThreads=false`, so the Java 17 fallback — its naming, its daemon status, and its unboundedness under 64 concurrent blocking tasks — is covered even when the build runs on 25.

## [0.1.25] - 2026-08-20

### Breaking
- **`McpSdkServerConfig.instance()` is now an `McpMessageHandler`, not an `SdkMcpServer`.** `McpSdkServerConfig` is a record, so both the accessor's return type *and* the canonical constructor's descriptor changed: code compiled against 0.1.24 that reads or constructs one needs a recompile. Anyone using `SdkMcpServer.toConfig()` — the documented path — is unaffected. The widening is what makes "bring your own MCP server" possible; see the new SPI below.
- **An unknown tool is now an `isError` result, not a JSON-RPC `-32601`.** The text is Python's verbatim: `Tool '<name>' not found`. The previous behaviour was argued from the MCP specification's error-handling section, which does list unknown tools as protocol errors — but the practical point decides it: an `isError` result reaches the model, which can read it and correct itself, while a JSON-RPC error never does. This removes the last deliberate SDK-MCP divergence from the Python SDK.
- **A tool whose `inputSchema` is not valid JSON Schema is no longer callable.** Validation now fails **closed**, as Python's does. Each schema is checked against its own dialect's meta-schema when the server is built; a tool that fails is logged at `WARNING` and every call to it returns `isError` without running the handler. This is not hypothetical strictness: the validator accepts a malformed schema and then mis-validates against it — `{"type": "bogus"}` compiles and matches nothing, so every call failed citing the caller's arguments rather than the real defect, and `"properties": "a string"` was ignored outright so every call was waved through unchecked. Unknown/vendor keywords remain legal.

### Added
- **`notifications/cancelled` cancels a running tool** (the behaviour Python gained in PR #1218). The CLI does send this — verified live: with `MCP_TOOL_TIMEOUT=5000` and a sleeping tool, an SDK MCP server receives `[initialize, notifications/initialized, tools/list, tools/call, notifications/cancelled]`. The pending call is now answered `-32800 Request cancelled` and any late result is discarded. Because `CompletableFuture.cancel(true)` does **not** interrupt a running task, a handler can only stop if it can see the cancellation: the new **`ToolCallContext`** is that signal, taken as an optional second handler parameter (`(args, context) -> ...`), with `isCancelled()`, `onCancel(Runnable)` and `throwIfCancelled()`. Existing one-argument handlers compile and behave exactly as before. A `@Tool`-annotated method may declare a `ToolCallContext` parameter anywhere in its signature; it is injected and never appears in the published schema.
- **`McpMessageHandler`**, the SPI `McpSdkServerConfig` now holds. Implement it to serve parts of MCP that `SdkMcpServer` does not — resources, prompts, completions — or to adapt a third-party MCP library, without the SDK taking that dependency for everyone. Its `handleMessage` returns `null` for a message that expects no reply, and `default void close()` signals that the connection using the handler is going away (idempotent; the handler stays usable for the next one).
- **`ping` is answered.** It was `-32601`, which a conformant client may read as a dead connection.
- **MCP protocol version negotiation.** The client's requested version was previously ignored and `2024-11-05` returned unconditionally. The server now advertises `2025-06-18` and `2024-11-05` and echoes a requested version it speaks, else answers with the newest it does. `2025-03-26` is deliberately **not** claimed: it made JSON-RPC batching mandatory to receive, and a batch is a top-level array, which `SDKControlMcpMessageRequest.message` types as a map and cannot represent.
- **`ToolResult.json(...)`, `ToolResult.ofContent(...)`, `addJson`, `addResourceLink`, `addResource`, `addContent`.** `json(...)` and a raw-content factory were documented but did not exist. The content factories port Python's `_convert_tool_content`: text and image pass through, a `resource_link` flattens to its name/URI/description, a text resource contributes its text, and binary or unrecognized blocks are dropped with a warning.

### Fixed
- **A notification no longer gets a JSON-RPC response.** Only `notifications/initialized` was special-cased (and it replied with a malformed `{jsonrpc, result}` carrying no `id`); every other notification was answered `-32601` — an error response to a notification, which JSON-RPC forbids. Notifications now produce no reply at all, and the control request that carried one is acknowledged with `{"jsonrpc": "2.0", "result": {}}`, matching Python.
- **A message with no `method` no longer throws.** `switch (method)` on a null threw `NullPointerException` straight out of `handleMessage`, bypassing all the error handling below it; a non-`Map` `params` threw `ClassCastException` the same way. A message with no method is a JSON-RPC response and is now ignored (this server sends the CLI no requests), and malformed `params` is `-32602`.
- **`"arguments": null` no longer reaches the handler as null.** `getOrDefault` returns the *stored* null, so a handler was handed null and typically NPE'd on it. Absent, explicitly null, and non-object all mean no arguments now, as in Python.
- **A tool's `title` is no longer dropped when it declares no annotations.** `tools/list` skipped the whole annotations block when `annotations()` was null — but that block is what emits the title. The title is now sent both at the top level (where MCP 2025-06-18 puts it) and inside `annotations` (where earlier revisions look).
- **An unknown SDK MCP server, or a handler that fails, no longer fails the whole control request.** Both threw, producing a `control_response` error. They are now reported as JSON-RPC `-32601` / `-32603` *inside* `mcp_response`, so one MCP call fails and the control channel stays healthy — the Python SDK's behaviour.
- **The 60-second cap on an SDK MCP tool is gone.** It did not do what it looked like: on timeout the tool kept running (the future was abandoned, never cancelled) and the failure was reported as a control-protocol error, turning "slow" into "broken" while bounding nothing. The CLI already owns this timeout and now signals it, and `close()` drains whatever is left, so there are two real escape hatches where there were none. Python has no SDK-side cap either.
...
### Added
- **SDK MCP tool arguments are validated against the tool's `inputSchema`** before the handler runs, as the MCP specification requires of servers ("Servers **MUST** validate all tool inputs"). A call that does not match the schema comes back as a `tools/call` result with `isError: true` and the text `Input validation error: …`, and the handler is never invoked. Previously nothing was validated: the arguments went straight to the handler, which typically failed on them and leaked JVM internals to the model — `class java.lang.String cannot be cast to class java.lang.Number (… loader 'bootstrap')` for a mistyped argument, an NPE message for a missing one. Neither tells a model what to fix, and a tool with side effects could have half-applied one before throwing. New dependency: `com.networknt:json-schema-validator`, pinned to the 2.x line because 3.x is built against Jackson 3 and would add a second JSON stack beside the Jackson 2 this SDK ships. It brings `slf4j-api` transitively — see the SLF4J note in `CLAUDE.md`; the SDK deliberately ships no logging binding.
...
### Changed
- **A tool handler that throws is now reported as an `isError` result, not a JSON-RPC error.** A tool that fails while running has produced a result, not an unprocessable request, and the specification reports those "in tool results with `isError: true`" so the model can read the failure and adapt. In practice the CLI already degraded the old `-32603` into an `isError` result, so the visible change is that the text arrives verbatim instead of prefixed with `Tool invocation failed: ` — confirmed against CLI 2.1.235, where a throwing tool and a tool returning `isError` are now indistinguishable to the model. An unknown tool is still answered with `-32601`: the specification lists "Unknown tools" under protocol errors, so Java keeps that deliberately, diverging from Python.

  **Breaking for anyone asserting on the wire shape:** a failing `tools/call` no longer produces a JSON-RPC `error` object. `SdkMcpTest.testServerErrorHandling` was updated accordingly.
